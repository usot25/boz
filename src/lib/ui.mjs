...
const consentScript = () => {
  ...
  const id = esc(consent.domainScriptId);

  const autoBlock = consent.autoBlock
    ? `<script type="text/javascript" src="https://cdn.cookielaw.org/consent/${id}/OtAutoBlock.js"></script>\n`
    : '';

  return `${autoBlock}<script src="https://cdn.cookielaw.org/scripttemplates/otSDKStub.js" type="text/javascript" charset="UTF-8" data-domain-script="${id}"></script>
